...
if (!(isset($_SESSION['auth']['is_super_user']) && ($_SESSION['auth']['is_super_user'])) or $is_demo) {
header ("HTTP/1.0 403 Forbidden"); exit;
}

$cpr='&copy; Sypex Dumper ';
...
elseif (!empty($_COOKIE['sxd'])) {
    ...
    if (isset($_SESSION['auth']['is_super_user']) && ($_SESSION['auth']['is_super_user'])) {
        if (@mysql_connect(DBHOST, $dbconfig['username'], $dbconfig['password'])) {
            setcookie("sxd", base64_encode("SKD101:{$dbconfig['username']}:{$dbconfig['password']}"));
            $db_auth = 1;
            mysql_close();
            exit;
        }
...
if (!$db_auth || (isset($_SERVER['QUERY_STRING']) && $_SERVER['QUERY_STRING'] == 'reload')) {
    setcookie("sxd");
    echo tpl_page(tpl_auth($error ? tpl_error($error) : ''), "<SCRIPT>if (jsEnabled) {document.write('<INPUT TYPE=submit VALUE=Применить>');}</SCRIPT>");
    echo "<SCRIPT>document.getElementById('timer').innerHTML = '" .$cpr. round(array_sum(explode(' ', microtime())) - $timer, 4) . " сек.'</SCRIPT>";
    exit;
}
if (!file_exists(PATH) && !$is_safe_mode) {
    mkdir(PATH, 0777) || trigger_error(_("Не удалось создать каталог для бекапа"), E_USER_ERROR);
}
...
class dumper {
    ...
    function backup() {
        ...
        $buttons = "<A ID=save HREF='' STYLE='display: none;' target='_blank'>"._("Скачать файл")."</A> &nbsp; <INPUT ID=back TYPE=button VALUE='"._("Вернуться")."' DISABLED onClick=\"history.back();\">";
        echo tpl_page(tpl_process(_("Создается резервная копия БД")), $buttons);
        ...
        if (!$db) {
            echo tpl_l(_("ОШИБКА! Не указана база данных!"), C_ERROR);
            echo tpl_enableBack();
            exit;
        }
        echo tpl_l(strtr(_("Подключение к БД `%dbname`."), array('%dbname'=>$db)));
        mysql_select_db($db) or trigger_error (_("Не удается выбрать базу данных.")."<BR>" . mysql_error(), E_USER_ERROR);
        $tables = array();
        ...
        echo tpl_l(_("Создание файла с резервной копией БД:")."<BR>\\n  -  {$this->filename}");
        ...
        if ($this->mysql_version > 40101 && CHARSET != 'auto') {
            mysql_query("SET NAMES '" . CHARSET . "'") or trigger_error (_("Неудается изменить кодировку соединения.")."<BR>" . mysql_error(), E_USER_ERROR);
        ...
        // Выставляем кодировку соединения соответствующую кодировке таблицы
            if ($this->mysql_version > 40101 && $tab_charset[$table] != $last_charset) {
                if (CHARSET == 'auto') {
                    mysql_query("SET NAMES '" . $tab_charset[$table] . "'") or trigger_error (_("Неудается изменить кодировку соединения.")."<BR>" . mysql_error(), E_USER_ERROR);
                    echo tpl_l(strtr(_("Установлена кодировка соединения `%encode`."),array('%encode'=>$tab_charset[$table])), C_WARNING);
                    $last_charset = $tab_charset[$table];
                }
                else {
                    echo tpl_l(_('Кодировка соединения и таблицы не совпадает:'), C_ERROR);
                    echo tpl_l(strtr('Таблица `%table` -> %tab_charset (соединение %CHARSET)', array('%table'=>$table, '%CHARSET'=>CHARSET, '%tab_charset'=>$tab_charset[$table])), C_ERROR);
                }
            }
            echo tpl_l(strtr(_("Обработка таблицы `%table`"),array('%table'=>$table))." [" . fn_int($tabinfo[$table]) . "].");
...
#            $this->fn_write($fp, "DROP TABLE IF EXISTS `{$table}`;\n{$tab[1]};\n\n");
            ...
            //Очистка таблицы
            $this->fn_write($fp, "TRUNCATE TABLE `{$table}`;\n");
            ...
            if ($tabinfo[$table] > 0) {
                ...
                $i = 0;
                $this->fn_write($fp, "INSERT INTO `{$table}` ({$ColsList}) VALUES");
        ...
        echo tpl_l(str_repeat("-", 60));
        $this->fn_close($fp);
        echo tpl_l(strtr(_("Резервная копия БД `%db` создана."), array('%db'=>$db)), C_RESULT);
        echo tpl_l(strtr(_("Размер БД:       %size МБ"), array('%size'=>round($this->size / 1048576, 2))), C_RESULT);
        $filesize = strtr(_("%size МБ"), array('%size'=>round(filesize(PATH . $this->filename) / 1048576, 2)));
        echo tpl_l(_("Размер файла:")." {$filesize}", C_RESULT);
        echo tpl_l(_("Таблиц обработано:")." {$tabs}", C_RESULT);
        echo tpl_l(_("Строк обработано:")."   " . fn_int($tabinfo[0]), C_RESULT);
        echo "<SCRIPT>with (document.getElementById('save')) {style.display = ''; innerHTML = '"._("Скачать файл")." ({$filesize})'; href = '" . URL . $this->filename . "'; }document.getElementById('back').disabled = 0;</SCRIPT>";
    }

    function restore() {
        if (!isset($_POST)) {$this->main();}
        set_error_handler("SXD_errorHandler");
        $buttons = "<INPUT ID=back TYPE=button VALUE='"._("Вернуться")."' DISABLED onClick=\"history.back();\">";
        echo tpl_page(tpl_process(_("Восстановление БД из резервной копии")), $buttons);
        ...
        if(preg_match("/^(.+?)\.sql(\.(bz2|gz))?$/", $file, $matches)) {
            ...
            if (!file_exists(PATH . "/{$file}")) {
                echo tpl_l(_("ОШИБКА! Файл не найден!"), C_ERROR);
                echo tpl_enableBack();
                exit;
            }
            echo tpl_l(strtr(_("Чтение файла `%filename`."),array('%filename'=>$file)));
            $file = $matches[1];
        }
        else {
            echo tpl_l(_("ОШИБКА! Не выбран файл!"), C_ERROR);
            echo tpl_enableBack();
            exit;
        }
        echo tpl_l(str_repeat("-", 60));
        ...
        if ($this->mysql_version > 40101 && (CHARSET != 'auto' || $this->forced_charset)) { // Кодировка по умолчанию, если в дампе не указана кодировка
            mysql_query("SET NAMES '" . $this->restore_charset . "'") or trigger_error (_("Неудается изменить кодировку соединения.")."<BR>" . mysql_error(), E_USER_ERROR);
            echo tpl_l(strtr(_("Установлена кодировка соединения `%encode`."),array('%encode'=>$this->restore_charset)), C_WARNING);
        ...
        while(($str = $this->fn_read_str($fp)) !== false) {
            ...
            if (!$insert && preg_match("/^(INSERT INTO `?([^` ]+)`? .*?VALUES)(.*)$/i", $str, $m)) {
                if ($table != $m[2]) {
                    $table = $m[2];
                    $tabs++;
                    $cache .= tpl_l(strtr(_("Таблица `%table`."), array('%table'=>$table)));
                    $last_showed = $table;
                    $i = 0;
                    if ($is_skd)
            ...
            if ($sql) {
                        ...
                        // Выставляем кодировку соединения
                            if (preg_match("/(CHARACTER SET|CHARSET)[=\s]+(\w+)/i", $sql, $charset)) {
                                if (!$this->forced_charset && $charset[2] != $last_charset) {
                                    if (CHARSET == 'auto') {
                                        mysql_query("SET NAMES '" . $charset[2] . "'") or trigger_error (_("Неудается изменить кодировку соединения.")."<BR>{$sql}<BR>" . mysql_error(), E_USER_ERROR);
                                        $cache .= tpl_l(strtr(_("Установлена кодировка соединения `%encode`."),array('%encode'=>$charset[2])), C_WARNING);
                                        $last_charset = $charset[2];
                                    }
                                    else {
                                        $cache .= tpl_l(_('Кодировка соединения и таблицы не совпадает:'), C_ERROR);
										$cache .= tpl_l(strtr('Таблица `%table` -> %tab_charset (соединение %CHARSET)', array('%table'=>$table, '%CHARSET'=>$this->restore_charset, '%tab_charset'=>$charset[2])), C_ERROR);
                                    }
                                }
                            ...
                            elseif(CHARSET == 'auto') { // Вставляем кодировку для таблиц, если она не указана и установлена auto кодировка
                                ...
                                if ($this->restore_charset != $last_charset) {
                                    ...
                                    $cache .= tpl_l(strtr(_("Установлена кодировка соединения `%encode`."),array('%encode'=>$this->restore_charset)), C_WARNING);
                                    $last_charset = $this->restore_charset;
                                }
                            }
                        }
                        if ($last_showed != $table) {$cache .= tpl_l(strtr(_("Таблица `%table`."), array('%table'=>$table))); $last_showed = $table;}
                    }
                ...
                if ($execute) {
                    $q++;
                    mysql_query($sql) or trigger_error (_("Неправильный запрос.")."<BR>" . mysql_error(), E_USER_ERROR);
        ...
        echo tpl_s(1 , 1);
        echo tpl_l(str_repeat("-", 60));
        echo tpl_l(_("БД восстановлена из резервной копии."), C_RESULT);
        if (isset($info[3])) echo tpl_l(_("Дата создания копии:")." {$info[3]}", C_RESULT);
        echo tpl_l(_("Запросов к БД:")." {$q}", C_RESULT);
        echo tpl_l(_("Таблиц создано:")." {$tabs}", C_RESULT);
        echo tpl_l(_("Строк добавлено:")." {$aff_rows}", C_RESULT);
    ...
    function main() {
        $this->comp_levels = array('9' => '9 ('._('максимальная').')', '8' => '8', '7' => '7', '6' => '6', '5' => '5 (средняя)', '4' => '4', '3' => '3', '2' => '2', '1' => '1 ('._('минимальная').')','0' => _('Без сжатия'));
        ...
        $this->comp_methods[0] = _('Без сжатия');
        if (count($this->comp_methods) == 1) {
            $this->comp_levels = array('0' =>_('Без сжатия'));
        }
		...
		if ($_GET['act']=='restore')
			$buttons = "<INPUT TYPE=submit VALUE="._("Применить")." onclick='if(!confirm(\""._("Внимание! После восстановления базы данных вы потеряете все изменения, сделанные с момента бэкапа! Вы уверены, что хотите сделать восстановление?")."\")) return false;'>";
		else
			$buttons = "<INPUT TYPE=submit VALUE="._("Применить").">";
...
<BODY TEXT=#000000>
<TABLE WIDTH=100% HEIGHT=100% BORDER=0 CELLSPACING=0 CELLPADDING=0 ALIGN=CENTER style='position: absolute; left: 0pt; top: 0pt;'>
<TR>
...
<TD VALIGN=TOP BGCOLOR=#F6F4E6 STYLE="FILTER: progid:DXImageTransform.Microsoft.Gradient(gradientType=0,startColorStr=#FCFBFE,endColorStr=#F6F4E6); padding: 8px 8px;">
    {$content}
...
function tpl_main() {
    ...
    $fieldset=(($_GET['act']=='backup')?"
<FIELDSET>
<LEGEND>
"._("Backup / Создание резервной копии БД")."&nbsp;</LEGEND>
<input type=hidden name=db_backup value='{$dbconfig['dbname']}'>
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
...
".(count($SK->comp_methods) > 1?
"<TR>
<TD>"._("Метод сжатия:")."</TD>
<TD style='width:70%'>
...
":"
<input type=hidden name=comp_method value=0 />").
(count($SK->comp_levels) > 1?"
<TR>
<TD>"._("Степень сжатия:")."</TD>
<TD>
...
":"
<input type=hidden name=comp_level value=0 />")."
</TABLE>
</FIELDSET>
        ":"
<FIELDSET>
<LEGEND>
"._("Restore / Восстановление БД из резервной копии")."&nbsp;</LEGEND>
<input type=hidden name=db_restore value='{$dbconfig['dbname']}'>
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
...
<TD WIDTH=65%><SELECT NAME=file>
    ...
    return <<<HTML
<INPUT TYPE=hidden NAME=action VALUE={$_GET['act']}>
    {$fieldset}
</SPAN>
<!--<SCRI PT>
...
<TR><TD WIDTH=31%>"._("Статус таблицы:")."</TD><TD WIDTH=69%><TABLE WIDTH=100% BORDER=1 CELLPADDING=0 CELLSPACING=0>
...
<TR><TD>"._("Общий статус:")."</TD><TD><TABLE WIDTH=100% BORDER=1 CELLSPACING=0 CELLPADDING=0>
...
<SPAN ID=error>
<FIELDSET>
<LEGEND>"._("Ошибка")."</LEGEND>
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
<TD>"._("Для работы Sypex Dumper Lite требуется:<BR> - Internet Explorer 5.5+, Mozilla либо Opera 8+")." (<SPAN ID=sie>-</SPAN>)<BR> - "._("включено выполнение JavaScript скриптов")." (<SPAN ID=sjs>-</SPAN>)</TD>
</TR>
</TABLE>
</FIELDSET>
</SPAN>
<SPAN ID=body STYLE=\"display: none;\">
    {$error}
<FIELDSET>
<LEGEND>"._("Введите логин и пароль")."</LEGEND>
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
<TD WIDTH=41%>"._("Логин:")."</TD>
...
function tpl_backup_index() {
    return "
    <CENTER>
<H1>"._("У вас нет прав для просмотра этого каталога")."</H1>
</CENTER>
...
function tpl_error($error) {
    return "
<FIELDSET>
<LEGEND>"._("Ошибка при подключении к БД")."</LEGEND>
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
...
function SXD_errorHandler($errno, $errmsg, $filename, $linenum, $vars) {
    ...
    echo tpl_l("{$dt}<BR><B>"._("Возникла ошибка!</B>"), C_ERROR);
