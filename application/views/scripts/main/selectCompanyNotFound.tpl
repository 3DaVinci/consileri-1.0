...
{if $company==""}{assign var="company" value=$smarty.get.company}{/if}
{literal}
<script>
...
var nowPage = "selectCompanyNotFound?company={$selectedCompany|escape:'url'}&page={$page}";
...
<span class="size12" style="margin-left:23px;">{t company=$company}Компании '%company' нет в нашей базе!{/t} 
<a href="{$siteurl}/main/addCompany?name={$company|escape:'url'}" class="cLink">{t}Добавить?{/t}</a>
</span>
...
<script type="text/javascript">
    var pFrom   = 1,
        ...
        curPage = {$page},
        timer, timer2,
        page = '{$siteurl}/main/selectCompanyNotFound?company={$company|escape:'url'}&page=';
</script>
...
<tr>
  ...
  <td colspan="2" nowrap>
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  <a href="{$siteurl}/main/selectCompanyNotFound?company={$company|escape:'url'}" class="pagesAllList"  onclick="if ({$pages}>50 && !confirm('{t}Вывод всех данных может выполняться долго!\nВы уверены?{/t}')) return false;">{t}весь список{/t}</a>   
  </td>
</tr>
<tr>
...
{else}
	...
	{if $smarty.section.name.index==$page} 
		...
		{else}
		<span class="pageInactive"><a href="{$siteurl}/main/selectCompanyNotFound?company={$company|escape:'url'}&page={$smarty.section.name.index}" class="pageInactiveLink">{$smarty.section.name.index}</a></span>
		{/if}
	{/section}
	&nbsp;&nbsp;&nbsp;
	<a href="{$siteurl}/main/selectCompanyNotFound?company={$selectedCompany|escape:'url'}" class="pagesAllList">{t}весь список{/t}</a>
{/if}
...
<input type="button" value="Добавить" class="hisBtn" tabindex="3" id="t2" onclick="addHistoryReq({$cur.prop.id})"><br>
...
{else} {* чужая компания *}
<div class="companyBriefWrap" style="background-image:none">
<div class="companyBriefShort">
<b class="cNameShortLink">{$cur.prop.name}</b><br>
{t}Копанию ведет менеджер:{/t} <b>{$cur.manager}</b><br>
...
{else}
	...
	{if $smarty.section.name.index==$page} 
		<span class="pageActive">{$smarty.section.name.index}</span>
		{else}
		<span class="pageInactive"><a href="/main/selectCompanyNotFound?company={$company|escape:'url'}&page={$smarty.section.name.index}" class="pageInactiveLink">{$smarty.section.name.index}</a></span>
		{/if}
	{/section}
	&nbsp;&nbsp;&nbsp;
	<a href="{$siteurl}/main/selectCompanyNotFound?company={$company|escape:'url'}" class="pagesAllList">{t}весь список{/t}</a>
{/if}
